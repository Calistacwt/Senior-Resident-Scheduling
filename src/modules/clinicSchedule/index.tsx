...
//   // Fetch the clinic schedule when the component mounts
//   useEffect(() => {
...
const ClinicSchedule: React.FC = () => {
  ...
  const [importedData, setImportedData] = useState<clinicSchedule[]>([]); // State for imported data
  const fileInputRef = useRef<HTMLInputElement | null>(null); // Create a ref for the file input
  ...
  useEffect(() => {
    const getSchedule = async () => {
      const scheduleData = await fetchClinicSchedule();
      setFetchedData(scheduleData); // Only set the fetched data
    };
